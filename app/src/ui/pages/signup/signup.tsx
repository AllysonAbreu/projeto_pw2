...
const Signup: React.FC = () => {
  const [credenciaisRegistro, setCredenciaisRegistro] = useState(
    CREDENCIAIS_INICIAIS_REGISTRO_USUARIO_STATE
  );
  const [erro, setErro] = useState(CREDENCIAIS_INICIAIS_ERRO_STATE);

  const { addToast } = useContext(ToastifyContext);
  const { register, updateImage } = useUserApi();
  ...
  const imageName = credenciaisRegistro.imagem?.name ? credenciaisRegistro.imagem?.name : 'foto';
  ...
  const handleRegister = async (event: React.FormEvent) => {
    ...
    if (validateForm) {
      try {
        const responseUSer = await register(credenciaisRegistro);
        await updateImage(responseUSer.response.id, imageName, credenciaisRegistro.imagem);
  ...
  return (
    <div className="page">
      <div className="signuptitle">Crie sua conta</div>
      <div className="signupcontainer">
        ...
        <form className="form-singup" onSubmit={handleRegister}>
          ...
          <div className="input-row">
            ...
            <div>
              <div className="signup-gray-text">Foto</div>
              <input
                type="file"
                name="imagem"
                onChange={handleInputChange}
